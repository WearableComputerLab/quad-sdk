...
/**
  ...
  \image html fig_Constraint_Loop.png "A loop constraint restricts the movement
  of a point p located on the predecessor body relative to point s on a
  successor body in the directions T_i."
    ...
    \f$p\f$ and \f$s\f$ in the spatial directions \f$^pT_i\f$. The user
  specifies \f$p\f$ and \f$s\f$ by passing in the spatial transform from the
  origin of the predecessor body to the predecessor frame (\f$_{1}^{1}r_{P}\f$,
...
class RBDL_DLLAPI LoopConstraint : public Constraint {
  ...
  */
  LoopConstraint(
      const unsigned int bodyIdPredecessor, const unsigned int bodyIdSuccessor,
      ...
      bool positionLevelConstraint = true, bool velocityLevelConstraint = true);
  ...
  */
  const std::vector<Math::SpatialVector> &getConstraintAxes() { return T; }

...
/**
  ...
  \image html fig_Constraint_Contact.png "A contact constraint restricts the
  movement of a point p located on a body such that its velocity and
  acceleration in direction n_i is zero."
    ...
    \f$_{1}^{1}r_{P}\f$ (the vector from the origin of the body's frame, to
  point \f$p\f$, resolved in the coordinates of the body's frame) and the normal
...
class RBDL_DLLAPI ContactConstraint : public Constraint {
  ...
  */
  ...
  ContactConstraint(
      const unsigned int bodyId, const Math::Vector3d &bodyPoint,
  ...
  void calcConstraintJacobian(Model &model, const double time,
                              const Math::VectorNd &Q,
                              const Math::VectorNd &QDot,
                              Math::MatrixNd &GSysUpd, ConstraintCache &cache,
                              bool updateKinematics = false) override;

  void calcGamma(Model &model, const double time, const Math::VectorNd &Q,
                 const Math::VectorNd &QDot, const Math::MatrixNd &GSys,
                 Math::VectorNd &gammaSysUpd, ConstraintCache &cache,
                 bool updateKinematics = false) override;

  void calcPositionError(Model &model, const double time,
                         const Math::VectorNd &Q, Math::VectorNd &errSysUpd,
                         ConstraintCache &cache,
                         bool updateKinematics = false) override;

  void calcVelocityError(Model &model, const double time,
                         const Math::VectorNd &Q, const Math::VectorNd &QDot,
                         const Math::MatrixNd &GSys, Math::VectorNd &derrSysUpd,
                         ConstraintCache &cache,
                         bool updateKinematics = false) override;
  ...
  */
  const std::vector<Math::Vector3d> &getConstraintNormalVectors() { return T; }
...
*/
  ...
  */
  ...
  /*** @brief To support ForwardDynamicsKokkevis
    ...
    @param QDot: the generalized velocities of the model
    @param QDDot: the generalized accelerations of the model
    @param pointAccelerationUpd the acclerations of the one point in this
    constraint
    @param updateKinematics setting this to true will force kinematic functions
            to use the values of Q, QDot, and QDDot passed into this function.
  ...
  */
  ...
  void calcPointAccelerations(Model &model, const Math::VectorNd &Q,
                              const Math::VectorNd &QDot,
                              const Math::VectorNd &QDDot,
  ...
  */
  void calcPointForceJacobian(Model &model, const Math::VectorNd &Q,
                              ConstraintCache &cache,

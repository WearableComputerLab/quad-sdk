...
/**
  ...
  \f$\phi_p(q,t)=0\f$, the velocity-level \f$\phi_v(\dot{q},q,t)=0\f$, or the
  acceleration-level \f$\phi_a(\ddot{q},\dot{q},q,t)=0\f$. These constraints all
  end up being applied at the acceleration-level by taking successive
  derivatives until we are left with \f$\Phi(\ddot{q},\dot{q},q,t)=0\f$. A new
  concrete Constraint class must provide implementations for the following
  methods:
  ...
  - <b> Physical correctness (\f$G\f$ and \f$\gamma\f$): Agrees with equivalent
  joint-coordinate model?</b> Evaluate \f$\ddot{q}\f$ by calling
  ForwardDynamicsConstraintsDirect on your system. If possible construct a model
  that uses joint coordinates that is perfectly equivalent to the model that
  uses constraints and evaluate its forward dynamics using the same initial
  state. The matching indices of \f$\ddot{q}\f$ should be identical. Examples of
  tests like this can be seen in ConstraintCorrectnessTest in
  ...
  - <b>Compare against known errors (\f$\epsilon_{P}\f$ and \f$\epsilon_{V}\f$)
  </b> The values returned for the position and velocity errors of the
  constraint are most easily evaluated by constructing a system in which the
  constraint is satisfied at the position and velocity level and then calling
...
class RBDL_DLLAPI Constraint {
  ...
  */
  virtual void calcConstraintJacobian(Model &model, const double time,
  ...
  /**
    ...
    @param updateKinematics: setting this flag to true will cause all
        ...
        generalized coordinates passed into this function.
  */
  virtual void calcGamma(Model &model, const double time,
                         const Math::VectorNd &Q, const Math::VectorNd &QDot,
                         const Math::MatrixNd &GSys,
                         Math::VectorNd &gammaSysOutput, ConstraintCache &cache,
                         bool updateKinematics = false) = 0;
  ...
  sizeOfConstraint rows.
    ...
    @param updateKinematics: setting this flag to true will cause all
        ...
        generalized coordinates passed into this function.
  */
  virtual void calcPositionError(Model &model, const double time,
                                 const Math::VectorNd &Q,
  ...
  and has sizeOfConstraint rows.
    ...
    @param updateKinematics: setting this flag to true will cause all
        ...
        generalized coordinates passed into this function.
  */
  virtual void calcVelocityError(Model &model, const double time,
                                 const Math::VectorNd &Q,
                                 const Math::VectorNd &QDot,
                                 const Math::MatrixNd &GSys,
  ...
  */
  virtual void calcConstraintForces(
      Model &model, const double time, const Math::VectorNd &Q,
      const Math::VectorNd &QDot, const Math::MatrixNd &GSys,
      ...
      ConstraintCache &cache, bool resolveAllInRootFrame = false,
      bool updateKinematics = false) = 0;
  ...
  */
  Constraint(const char *nameOfConstraint, unsigned int typeOfConstraint,
             unsigned int sizeOfConstraint, unsigned int userDefinedIdNumber)
  ...
  */
  unsigned int getUserDefinedId() { return id; }

  /**
    @param userDefinedId: (optional) integer id for this constraint
  */
  void setUserDefinedId(unsigned int userDefinedId) { id = userDefinedId; }
  ...
  */
  ...
  /**
    @param bgParamsUpd: (output) the Baumgarte stabilization coefficients for
    this constraint. Note the velocity error coefficient is in the 0 index and
    the position error coefficient is in the 1st index.
  */
  ...
  */
  void getBaumgarteStabilizationForces(const Math::VectorNd &errPos,
    ...
    baumgarteForces =
        (-2 * baumgarteParameters[0] * errVel -
  ...
  */
  ...
  /**
    ...
    ConstraintType
  */
  unsigned int getConstraintType() { return typeOfConstraint; }

  /**
    @return the number of constraint equations in this constraint
  */
  unsigned int getConstraintSize() { return sizeOfConstraint; }

  /**
    @return the index of the first row in G in which this constraint's
            entries appear.
  */
  unsigned int getConstraintIndex() { return rowInSystem; }
  ...
  */
  bool isBaumgarteStabilizationEnabled() { return baumgarteEnabled; }
  ...
  */
  void enableConstraintErrorFromAccelerationLevel(
      unsigned int constraintSubIndex) {
    assert(constraintSubIndex < sizeOfConstraint);
    positionConstraint[constraintSubIndex] = false;
  ...
  */
  const std::vector<unsigned int> &getBodyIds() { return bodyIds; }

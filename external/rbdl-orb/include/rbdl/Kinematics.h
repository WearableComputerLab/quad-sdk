...
RBDL_DLLAPI Math::Matrix3d CalcBodyWorldOrientation(
    Model &model, const Math::VectorNd &Q, const unsigned int body_id,
...
bool InverseKinematics(Model &model, const Math::VectorNd &Qinit,
                       ...
                       double lambda = 0.01, unsigned int max_iter = 55);
...
struct RBDL_DLLAPI InverseKinematicsConstraintSet {
  ...
  double lambda;  /// Damping factor, the default value of 1.0e-6 is reasonable
                  /// for most problems
  unsigned int num_steps;  // The number of iterations performed
  unsigned int max_steps;  // Maximum number of steps (default 300), abort if
                           // more steps are performed.
  double step_tol;  // Step tolerance (default = 1.0e-12). If the computed step
                    // length is smaller than this value the algorithm
                    // terminates successfully (i.e. returns true). If
                    // error_norm is still larger than constraint_tol then this
                    // usually means that the target is unreachable.
  double constraint_tol;  // Constraint tolerance (default = 1.0e-12). If
                          // error_norm is smaller than this value the algorithm
                          // terminates successfully, i.e. all constraints are
                          // satisfied.
  ...
  unsigned int AddOrientationConstraint(
      unsigned int body_id, const Math::Matrix3d &target_orientation,
      float weight = 1.);

...
PointcloudProcessor::extractClusterCloudsFromPointcloud(
    Pointcloud::ConstPtr inputCloud) const {
  std::vector<pcl::PointIndices> clusterIndices =
      extractClusterIndicesFromPointcloud(inputCloud);
  ...
  for (const auto& indicesSet : clusterIndices) {
    Pointcloud::Ptr clusterCloud =
        makeCloudFromIndices(indicesSet.indices, inputCloud);
...
PointcloudProcessor::extractClusterIndicesFromPointcloud(
    Pointcloud::ConstPtr inputCloud) const {
  ...
  euclideanClusterExtraction.setMinClusterSize(
      params_->get().clusterExtraction_.minNumPoints_);
  euclideanClusterExtraction.setMaxClusterSize(
...
Pointcloud::Ptr PointcloudProcessor::makeCloudFromIndices(
    const std::vector<int>& indices, Pointcloud::ConstPtr inputCloud) const {
...
Pointcloud::Ptr PointcloudProcessor::downsampleInputCloud(
    Pointcloud::ConstPtr inputCloud) const {
...
void PointcloudProcessor::savePointCloudAsPcdFile(
    const std::string& filename, const Pointcloud& cloud) const {
  pcl::PCDWriter writer;
  pcl::PCLPointCloud2 pointCloud2;
  pcl::toPCLPointCloud2(cloud, pointCloud2);
  writer.write(filename, pointCloud2, Eigen::Vector4f::Zero(),
...
Pointcloud::Ptr PointcloudProcessor::applyRigidBodyTransformation(
  ...
  auto transformedCloud = grid_map_pcl::transformCloud(
      inputCloud, grid_map_pcl::getRigidBodyTransform(

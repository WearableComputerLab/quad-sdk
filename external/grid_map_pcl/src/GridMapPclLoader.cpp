...
void GridMapPclLoader::preProcessInputCloud() {
  ...
  if (params_->get().outlierRemoval_.isRemoveOutliers_) {
    auto filteredCloud =
        pointcloudProcessor_.removeOutliersFromInputCloud(workingCloud_);
    setWorkingCloud(filteredCloud);
  }

  if (params_->get().downsampling_.isDownsampleCloud_) {
    auto downsampledCloud =
        pointcloudProcessor_.downsampleInputCloud(workingCloud_);
    setWorkingCloud(downsampledCloud);
  }

  auto transformedCloud =
      pointcloudProcessor_.applyRigidBodyTransformation(workingCloud_);
...
void GridMapPclLoader::initializeGridMapGeometryFromInputCloud() {
  ...
  ROS_INFO_STREAM("Grid map dimensions: " << workingGridMap_.getLength()(0)
                                          << " x "
                                          << workingGridMap_.getLength()(1));
  ROS_INFO_STREAM("Grid map resolution: " << workingGridMap_.getResolution());
  ROS_INFO_STREAM("Grid map num cells: " << workingGridMap_.getSize()(0)
                                         << " x "
...
#endif
  // Iterate through grid map and calculate the corresponding height based on
  // the point cloud
  for (unsigned int linearIndex = 0; linearIndex < linearGridMapSize;
       ++linearIndex) {
...
void GridMapPclLoader::processGridMapCell(const unsigned int linearGridMapIndex,
  ...
  // Get grid map index from linear index and check if enough points lie within
  // the cell
  const grid_map::Index index(grid_map::getIndexFromLinearIndex(
      linearGridMapIndex, workingGridMap_.getSize()));

  Pointcloud::Ptr pointsInsideCellBorder(new Pointcloud());
  pointsInsideCellBorder = getPointcloudInsideGridMapCellBorder(index);
  const bool isTooFewPointsInCell =
      pointsInsideCellBorder->size() <
      params_->get().gridMap_.minCloudPointsPerCell_;
  if (isTooFewPointsInCell) {
    ROS_WARN_STREAM_THROTTLE(
        10.0, "Less than " << params_->get().gridMap_.minCloudPointsPerCell_
                           << " points in a cell");
    return;
  }

  (*gridMapData)(index(0), index(1)) =
      calculateElevationFromPointsInsideGridMapCell(pointsInsideCellBorder);
}
...
double GridMapPclLoader::calculateElevationFromPointsInsideGridMapCell(
  ...
  double minClusterHeight =
      *(std::min_element(clusterHeights.begin(), clusterHeights.end()));

  return minClusterHeight;
}

GridMapPclLoader::Pointcloud::Ptr
GridMapPclLoader::getPointcloudInsideGridMapCellBorder(
...
void GridMapPclLoader::savePointCloudAsPcdFile(

...
MAVPACKED(typedef struct __mavlink_sim_state_t {
  ...
  float yaw; /*<  Attitude yaw expressed as Euler angles, not recommended except
  ...
  float
      vn; /*< [m/s] True velocity in NORTH direction in earth-fixed NED frame*/
...
#define MAVLINK_MESSAGE_INFO_SIM_STATE                           \
  {                                                              \
    108, "SIM_STATE", 21, {                                      \
      {                                                          \
          "q1", NULL, MAVLINK_TYPE_FLOAT,                        \
          0,    0,    offsetof(mavlink_sim_state_t, q1)},        \
          {"q2", NULL, MAVLINK_TYPE_FLOAT,                       \
           0,    4,    offsetof(mavlink_sim_state_t, q2)},       \
          {"q3", NULL, MAVLINK_TYPE_FLOAT,                       \
           0,    8,    offsetof(mavlink_sim_state_t, q3)},       \
          {"q4", NULL, MAVLINK_TYPE_FLOAT,                       \
           0,    12,   offsetof(mavlink_sim_state_t, q4)},       \
          {"roll", NULL, MAVLINK_TYPE_FLOAT,                     \
           0,      16,   offsetof(mavlink_sim_state_t, roll)},   \
          {"pitch", NULL, MAVLINK_TYPE_FLOAT,                    \
           0,       20,   offsetof(mavlink_sim_state_t, pitch)}, \
          {"yaw", NULL, MAVLINK_TYPE_FLOAT,                      \
           0,     24,   offsetof(mavlink_sim_state_t, yaw)},     \
          {"xacc", NULL, MAVLINK_TYPE_FLOAT,                     \
           0,      28,   offsetof(mavlink_sim_state_t, xacc)},   \
          {"yacc", NULL, MAVLINK_TYPE_FLOAT,                     \
           0,      32,   offsetof(mavlink_sim_state_t, yacc)},   \
          {"zacc", NULL, MAVLINK_TYPE_FLOAT,                     \
           0,      36,   offsetof(mavlink_sim_state_t, zacc)},   \
          {"xgyro", NULL, MAVLINK_TYPE_FLOAT,                    \
           0,       40,   offsetof(mavlink_sim_state_t, xgyro)}, \
          {"ygyro", NULL, MAVLINK_TYPE_FLOAT,                    \
           0,       44,   offsetof(mavlink_sim_state_t, ygyro)}, \
          {"zgyro", NULL, MAVLINK_TYPE_FLOAT,                    \
           0,       48,   offsetof(mavlink_sim_state_t, zgyro)}, \
          {"lat", NULL, MAVLINK_TYPE_FLOAT,                      \
           0,     52,   offsetof(mavlink_sim_state_t, lat)},     \
          {"lon", NULL, MAVLINK_TYPE_FLOAT,                      \
           0,     56,   offsetof(mavlink_sim_state_t, lon)},     \
          {"alt", NULL, MAVLINK_TYPE_FLOAT,                      \
           0,     60,   offsetof(mavlink_sim_state_t, alt)},     \
          {"std_dev_horz",                                       \
           NULL,                                                 \
           MAVLINK_TYPE_FLOAT,                                   \
           0,                                                    \
           64,                                                   \
           offsetof(mavlink_sim_state_t, std_dev_horz)},         \
          {"std_dev_vert",                                       \
           NULL,                                                 \
           MAVLINK_TYPE_FLOAT,                                   \
           0,                                                    \
           68,                                                   \
           offsetof(mavlink_sim_state_t, std_dev_vert)},         \
          {"vn", NULL, MAVLINK_TYPE_FLOAT,                       \
           0,    72,   offsetof(mavlink_sim_state_t, vn)},       \
          {"ve", NULL, MAVLINK_TYPE_FLOAT,                       \
           0,    76,   offsetof(mavlink_sim_state_t, ve)},       \
          {"vd", NULL, MAVLINK_TYPE_FLOAT,                       \
           0,    80,   offsetof(mavlink_sim_state_t, vd)},       \
    }                                                            \
  }
#else
#define MAVLINK_MESSAGE_INFO_SIM_STATE                           \
 ...
 * @param roll  Attitude roll expressed as Euler angles, not recommended except
 * for human-readable outputs
 * @param pitch  Attitude pitch expressed as Euler angles, not recommended
 * except for human-readable outputs
 * @param yaw  Attitude yaw expressed as Euler angles, not recommended except
 * for human-readable outputs
...
static inline uint16_t mavlink_msg_sim_state_pack(
    uint8_t system_id, uint8_t component_id, mavlink_message_t* msg, float q1,
    float q2, float q3, float q4, float roll, float pitch, float yaw,
    float xacc, float yacc, float zacc, float xgyro, float ygyro, float zgyro,
    float lat, float lon, float alt, float std_dev_horz, float std_dev_vert,
...
static inline uint16_t mavlink_msg_sim_state_pack_chan(
    ...
    mavlink_message_t* msg, float q1, float q2, float q3, float q4, float roll,
    float pitch, float yaw, float xacc, float yacc, float zacc, float xgyro,
    float ygyro, float zgyro, float lat, float lon, float alt,
    float std_dev_horz, float std_dev_vert, float vn, float ve, float vd) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
  char buf[MAVLINK_MSG_ID_SIM_STATE_LEN];
  _mav_put_float(buf, 0, q1);
  _mav_put_float(buf, 4, q2);
  ...
  return mavlink_finalize_message_chan(
      msg, system_id, component_id, chan, MAVLINK_MSG_ID_SIM_STATE_MIN_LEN,
      MAVLINK_MSG_ID_SIM_STATE_LEN, MAVLINK_MSG_ID_SIM_STATE_CRC);
}
...
static inline uint16_t mavlink_msg_sim_state_encode(
  ...
  return mavlink_msg_sim_state_pack(
      system_id, component_id, msg, sim_state->q1, sim_state->q2, sim_state->q3,
      sim_state->q4, sim_state->roll, sim_state->pitch, sim_state->yaw,
      sim_state->xacc, sim_state->yacc, sim_state->zacc, sim_state->xgyro,
      sim_state->ygyro, sim_state->zgyro, sim_state->lat, sim_state->lon,
      sim_state->alt, sim_state->std_dev_horz, sim_state->std_dev_vert,
...
static inline uint16_t mavlink_msg_sim_state_encode_chan(
  ...
  return mavlink_msg_sim_state_pack_chan(
      system_id, component_id, chan, msg, sim_state->q1, sim_state->q2,
      sim_state->q3, sim_state->q4, sim_state->roll, sim_state->pitch,
      sim_state->yaw, sim_state->xacc, sim_state->yacc, sim_state->zacc,
      sim_state->xgyro, sim_state->ygyro, sim_state->zgyro, sim_state->lat,
      sim_state->lon, sim_state->alt, sim_state->std_dev_horz,
      sim_state->std_dev_vert, sim_state->vn, sim_state->ve, sim_state->vd);
}
...
static inline void mavlink_msg_sim_state_send(
    mavlink_channel_t chan, float q1, float q2, float q3, float q4, float roll,
    float pitch, float yaw, float xacc, float yacc, float zacc, float xgyro,
    float ygyro, float zgyro, float lat, float lon, float alt,
    float std_dev_horz, float std_dev_vert, float vn, float ve, float vd) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
  ...
  _mav_finalize_message_chan_send(
      chan, MAVLINK_MSG_ID_SIM_STATE, buf, MAVLINK_MSG_ID_SIM_STATE_MIN_LEN,
      MAVLINK_MSG_ID_SIM_STATE_LEN, MAVLINK_MSG_ID_SIM_STATE_CRC);
#else
  mavlink_sim_state_t packet;
  packet.q1 = q1;
  ...
  _mav_finalize_message_chan_send(
      chan, MAVLINK_MSG_ID_SIM_STATE, (const char*)&packet,
      MAVLINK_MSG_ID_SIM_STATE_MIN_LEN, MAVLINK_MSG_ID_SIM_STATE_LEN,
 ...
 */
static inline void mavlink_msg_sim_state_send_struct(
    mavlink_channel_t chan, const mavlink_sim_state_t* sim_state) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
  mavlink_msg_sim_state_send(
      chan, sim_state->q1, sim_state->q2, sim_state->q3, sim_state->q4,
      sim_state->roll, sim_state->pitch, sim_state->yaw, sim_state->xacc,
      sim_state->yacc, sim_state->zacc, sim_state->xgyro, sim_state->ygyro,
      sim_state->zgyro, sim_state->lat, sim_state->lon, sim_state->alt,
      sim_state->std_dev_horz, sim_state->std_dev_vert, sim_state->vn,
      sim_state->ve, sim_state->vd);
#else
  _mav_finalize_message_chan_send(
      chan, MAVLINK_MSG_ID_SIM_STATE, (const char*)sim_state,
      MAVLINK_MSG_ID_SIM_STATE_MIN_LEN, MAVLINK_MSG_ID_SIM_STATE_LEN,
      MAVLINK_MSG_ID_SIM_STATE_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_sim_state_send_buf(
    mavlink_message_t* msgbuf, mavlink_channel_t chan, float q1, float q2,
    float q3, float q4, float roll, float pitch, float yaw, float xacc,
    float yacc, float zacc, float xgyro, float ygyro, float zgyro, float lat,
    float lon, float alt, float std_dev_horz, float std_dev_vert, float vn,
    float ve, float vd) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
 ...
 * human-readable outputs
 */
static inline float mavlink_msg_sim_state_get_pitch(
    const mavlink_message_t* msg) {
 ...
 * human-readable outputs
 */
static inline float mavlink_msg_sim_state_get_yaw(
    const mavlink_message_t* msg) {
 ...
 */
static inline float mavlink_msg_sim_state_get_xacc(
    const mavlink_message_t* msg) {
 ...
 */
static inline float mavlink_msg_sim_state_get_zacc(
    const mavlink_message_t* msg) {
 ...
 */
static inline float mavlink_msg_sim_state_get_xgyro(
    const mavlink_message_t* msg) {
 ...
 */
static inline float mavlink_msg_sim_state_get_ygyro(
    const mavlink_message_t* msg) {
 ...
 */
static inline float mavlink_msg_sim_state_get_zgyro(
    const mavlink_message_t* msg) {
 ...
 * @return [m] Altitude
 */
static inline float mavlink_msg_sim_state_get_alt(
    const mavlink_message_t* msg) {
 ...
 */
static inline float mavlink_msg_sim_state_get_std_dev_horz(
    const mavlink_message_t* msg) {
 ...
 */
static inline float mavlink_msg_sim_state_get_std_dev_vert(
    const mavlink_message_t* msg) {
...
#else
  uint8_t len = msg->len < MAVLINK_MSG_ID_SIM_STATE_LEN
                    ? msg->len

...
MAVPACKED(typedef struct __mavlink_memory_vect_t {
...
#define MAVLINK_MESSAGE_INFO_MEMORY_VECT                           \
  {                                                                \
    249, "MEMORY_VECT", 4, {                                       \
      {"address", NULL, MAVLINK_TYPE_UINT16_T,                     \
       0,         0,    offsetof(mavlink_memory_vect_t, address)}, \
          {"ver", NULL, MAVLINK_TYPE_UINT8_T,                      \
           0,     2,    offsetof(mavlink_memory_vect_t, ver)},     \
          {"type", NULL, MAVLINK_TYPE_UINT8_T,                     \
           0,      3,    offsetof(mavlink_memory_vect_t, type)},   \
          {"value", NULL, MAVLINK_TYPE_INT8_T,                     \
           32,      4,    offsetof(mavlink_memory_vect_t, value)}, \
    }                                                              \
  }
#else
#define MAVLINK_MESSAGE_INFO_MEMORY_VECT                           \
 ...
 * @param type  Type code of the memory variables. for ver = 1: 0=16 x int16_t,
 * 1=16 x uint16_t, 2=16 x Q15, 3=16 x 1Q14
...
static inline uint16_t mavlink_msg_memory_vect_pack(
  ...
  return mavlink_finalize_message(
      msg, system_id, component_id, MAVLINK_MSG_ID_MEMORY_VECT_MIN_LEN,
...
static inline uint16_t mavlink_msg_memory_vect_pack_chan(
    ...
    mavlink_message_t* msg, uint16_t address, uint8_t ver, uint8_t type,
    const int8_t* value) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
  char buf[MAVLINK_MSG_ID_MEMORY_VECT_LEN];
  _mav_put_uint16_t(buf, 0, address);
  ...
  return mavlink_finalize_message_chan(
      msg, system_id, component_id, chan, MAVLINK_MSG_ID_MEMORY_VECT_MIN_LEN,
      MAVLINK_MSG_ID_MEMORY_VECT_LEN, MAVLINK_MSG_ID_MEMORY_VECT_CRC);
}
...
static inline uint16_t mavlink_msg_memory_vect_encode_chan(
    uint8_t system_id, uint8_t component_id, uint8_t chan,
    mavlink_message_t* msg, const mavlink_memory_vect_t* memory_vect) {
  return mavlink_msg_memory_vect_pack_chan(
      system_id, component_id, chan, msg, memory_vect->address,
...
static inline void mavlink_msg_memory_vect_send(mavlink_channel_t chan,
                                                uint16_t address, uint8_t ver,
                                                uint8_t type,
                                                const int8_t* value) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
  char buf[MAVLINK_MSG_ID_MEMORY_VECT_LEN];
  ...
  _mav_finalize_message_chan_send(
      chan, MAVLINK_MSG_ID_MEMORY_VECT, buf, MAVLINK_MSG_ID_MEMORY_VECT_MIN_LEN,
      MAVLINK_MSG_ID_MEMORY_VECT_LEN, MAVLINK_MSG_ID_MEMORY_VECT_CRC);
#else
  mavlink_memory_vect_t packet;
  packet.address = address;
  ...
  mav_array_memcpy(packet.value, value, sizeof(int8_t) * 32);
  _mav_finalize_message_chan_send(
      chan, MAVLINK_MSG_ID_MEMORY_VECT, (const char*)&packet,
      MAVLINK_MSG_ID_MEMORY_VECT_MIN_LEN, MAVLINK_MSG_ID_MEMORY_VECT_LEN,
 ...
 */
static inline void mavlink_msg_memory_vect_send_struct(
    mavlink_channel_t chan, const mavlink_memory_vect_t* memory_vect) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
  mavlink_msg_memory_vect_send(chan, memory_vect->address, memory_vect->ver,
                               memory_vect->type, memory_vect->value);
#else
  _mav_finalize_message_chan_send(
      chan, MAVLINK_MSG_ID_MEMORY_VECT, (const char*)memory_vect,
      MAVLINK_MSG_ID_MEMORY_VECT_MIN_LEN, MAVLINK_MSG_ID_MEMORY_VECT_LEN,
      MAVLINK_MSG_ID_MEMORY_VECT_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_memory_vect_send_buf(mavlink_message_t* msgbuf,
 ...
 *
 * @return  Type code of the memory variables. for ver = 1: 0=16 x int16_t, 1=16
 ...
 */
static inline void mavlink_msg_memory_vect_decode(
    const mavlink_message_t* msg, mavlink_memory_vect_t* memory_vect) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
...
#else
  uint8_t len = msg->len < MAVLINK_MSG_ID_MEMORY_VECT_LEN
                    ? msg->len

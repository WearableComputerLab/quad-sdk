...
MAVPACKED(typedef struct __mavlink_camera_image_captured_t {
  uint64_t time_utc; /*< [us] Timestamp (time since UNIX epoch) in UTC. 0 for
                        unknown.*/
...
#define MAVLINK_MESSAGE_INFO_CAMERA_IMAGE_CAPTURED                       \
  {                                                                      \
    263, "CAMERA_IMAGE_CAPTURED", 11, {                                  \
      {"time_boot_ms",                                                   \
       NULL,                                                             \
       MAVLINK_TYPE_UINT32_T,                                            \
       0,                                                                \
       8,                                                                \
       offsetof(mavlink_camera_image_captured_t, time_boot_ms)},         \
          {"time_utc",                                                   \
           NULL,                                                         \
           MAVLINK_TYPE_UINT64_T,                                        \
           0,                                                            \
           0,                                                            \
           offsetof(mavlink_camera_image_captured_t, time_utc)},         \
          {"camera_id",                                                  \
           NULL,                                                         \
           MAVLINK_TYPE_UINT8_T,                                         \
           0,                                                            \
           48,                                                           \
           offsetof(mavlink_camera_image_captured_t, camera_id)},        \
          {"lat", NULL, MAVLINK_TYPE_INT32_T,                            \
           0,     12,   offsetof(mavlink_camera_image_captured_t, lat)}, \
          {"lon", NULL, MAVLINK_TYPE_INT32_T,                            \
           0,     16,   offsetof(mavlink_camera_image_captured_t, lon)}, \
          {"alt", NULL, MAVLINK_TYPE_INT32_T,                            \
           0,     20,   offsetof(mavlink_camera_image_captured_t, alt)}, \
          {"relative_alt",                                               \
           NULL,                                                         \
           MAVLINK_TYPE_INT32_T,                                         \
           0,                                                            \
           24,                                                           \
           offsetof(mavlink_camera_image_captured_t, relative_alt)},     \
          {"q", NULL, MAVLINK_TYPE_FLOAT,                                \
           4,   28,   offsetof(mavlink_camera_image_captured_t, q)},     \
          {"image_index",                                                \
           NULL,                                                         \
           MAVLINK_TYPE_INT32_T,                                         \
           0,                                                            \
           44,                                                           \
           offsetof(mavlink_camera_image_captured_t, image_index)},      \
          {"capture_result",                                             \
           NULL,                                                         \
           MAVLINK_TYPE_INT8_T,                                          \
           0,                                                            \
           49,                                                           \
           offsetof(mavlink_camera_image_captured_t, capture_result)},   \
          {"file_url",                                                   \
           NULL,                                                         \
           MAVLINK_TYPE_CHAR,                                            \
           205,                                                          \
           50,                                                           \
           offsetof(mavlink_camera_image_captured_t, file_url)},         \
    }                                                                    \
  }
#else
#define MAVLINK_MESSAGE_INFO_CAMERA_IMAGE_CAPTURED                       \
  {                                                                      \
    "CAMERA_IMAGE_CAPTURED", 11, {                                       \
      {"time_boot_ms",                                                   \
       NULL,                                                             \
       MAVLINK_TYPE_UINT32_T,                                            \
       0,                                                                \
 ...
 * @param q  Quaternion of camera orientation (w, x, y, z order, zero-rotation
 * is 0, 0, 0, 0)
 * @param image_index  Zero based index of this image (image count since armed
 * -1)
 * @param capture_result  Boolean indicating success (1) or failure (0) while
 * capturing this image.
 * @param file_url  URL of image taken. Either local storage or http://foo.jpg
 * if camera provides an HTTP interface.
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_camera_image_captured_pack(
    uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
    uint32_t time_boot_ms, uint64_t time_utc, uint8_t camera_id, int32_t lat,
    int32_t lon, int32_t alt, int32_t relative_alt, const float* q,
    int32_t image_index, int8_t capture_result, const char* file_url) {
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_camera_image_captured_pack_chan(
    uint8_t system_id, uint8_t component_id, uint8_t chan,
    mavlink_message_t* msg, uint32_t time_boot_ms, uint64_t time_utc,
    uint8_t camera_id, int32_t lat, int32_t lon, int32_t alt,
    int32_t relative_alt, const float* q, int32_t image_index,
    int8_t capture_result, const char* file_url) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
  char buf[MAVLINK_MSG_ID_CAMERA_IMAGE_CAPTURED_LEN];
  _mav_put_uint64_t(buf, 0, time_utc);
  _mav_put_uint32_t(buf, 8, time_boot_ms);
 ...
 */
static inline uint16_t mavlink_msg_camera_image_captured_encode(
    uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
    const mavlink_camera_image_captured_t* camera_image_captured) {
  return mavlink_msg_camera_image_captured_pack(
      system_id, component_id, msg, camera_image_captured->time_boot_ms,
      camera_image_captured->time_utc, camera_image_captured->camera_id,
      camera_image_captured->lat, camera_image_captured->lon,
      camera_image_captured->alt, camera_image_captured->relative_alt,
      camera_image_captured->q, camera_image_captured->image_index,
      camera_image_captured->capture_result, camera_image_captured->file_url);
}
 ...
 * @param camera_image_captured C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_camera_image_captured_encode_chan(
    uint8_t system_id, uint8_t component_id, uint8_t chan,
    mavlink_message_t* msg,
    const mavlink_camera_image_captured_t* camera_image_captured) {
  return mavlink_msg_camera_image_captured_pack_chan(
      system_id, component_id, chan, msg, camera_image_captured->time_boot_ms,
      camera_image_captured->time_utc, camera_image_captured->camera_id,
      camera_image_captured->lat, camera_image_captured->lon,
      camera_image_captured->alt, camera_image_captured->relative_alt,
      camera_image_captured->q, camera_image_captured->image_index,
...
static inline void mavlink_msg_camera_image_captured_send(
    mavlink_channel_t chan, uint32_t time_boot_ms, uint64_t time_utc,
    uint8_t camera_id, int32_t lat, int32_t lon, int32_t alt,
    int32_t relative_alt, const float* q, int32_t image_index,
    int8_t capture_result, const char* file_url) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
  ...
  _mav_put_float_array(buf, 28, q, 4);
  _mav_put_char_array(buf, 50, file_url, 205);
  _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_CAMERA_IMAGE_CAPTURED,
                                  buf,
                                  MAVLINK_MSG_ID_CAMERA_IMAGE_CAPTURED_MIN_LEN,
                                  MAVLINK_MSG_ID_CAMERA_IMAGE_CAPTURED_LEN,
                                  MAVLINK_MSG_ID_CAMERA_IMAGE_CAPTURED_CRC);
 ...
 */
static inline void mavlink_msg_camera_image_captured_send_struct(
    mavlink_channel_t chan,
    const mavlink_camera_image_captured_t* camera_image_captured) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
  mavlink_msg_camera_image_captured_send(
      chan, camera_image_captured->time_boot_ms,
      camera_image_captured->time_utc, camera_image_captured->camera_id,
      camera_image_captured->lat, camera_image_captured->lon,
      camera_image_captured->alt, camera_image_captured->relative_alt,
      camera_image_captured->q, camera_image_captured->image_index,
...
static inline void mavlink_msg_camera_image_captured_send_buf(
    ...
    uint64_t time_utc, uint8_t camera_id, int32_t lat, int32_t lon, int32_t alt,
    int32_t relative_alt, const float* q, int32_t image_index,
    int8_t capture_result, const char* file_url) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
...
#else
  mavlink_camera_image_captured_t* packet =
      (mavlink_camera_image_captured_t*)msgbuf;
  packet->time_utc = time_utc;
  packet->time_boot_ms = time_boot_ms;
  packet->lat = lat;
  packet->lon = lon;
 ...
 */
static inline uint32_t mavlink_msg_camera_image_captured_get_time_boot_ms(
 ...
 */
static inline uint64_t mavlink_msg_camera_image_captured_get_time_utc(
    const mavlink_message_t* msg) {
 ...
 */
static inline uint8_t mavlink_msg_camera_image_captured_get_camera_id(
    const mavlink_message_t* msg) {
 ...
 */
static inline int32_t mavlink_msg_camera_image_captured_get_lat(
    const mavlink_message_t* msg) {
 ...
 */
static inline int32_t mavlink_msg_camera_image_captured_get_lon(
    const mavlink_message_t* msg) {
 ...
 */
static inline int32_t mavlink_msg_camera_image_captured_get_alt(
    const mavlink_message_t* msg) {
 ...
 */
static inline int32_t mavlink_msg_camera_image_captured_get_relative_alt(
    const mavlink_message_t* msg) {
 ...
 *
 * @return  Quaternion of camera orientation (w, x, y, z order, zero-rotation is
 * 0, 0, 0, 0)
 */
static inline uint16_t mavlink_msg_camera_image_captured_get_q(
 ...
 */
static inline int32_t mavlink_msg_camera_image_captured_get_image_index(
    const mavlink_message_t* msg) {
 ...
 *
 * @return  Boolean indicating success (1) or failure (0) while capturing this
 * image.
 */
static inline int8_t mavlink_msg_camera_image_captured_get_capture_result(
    const mavlink_message_t* msg) {
 ...
 */
static inline uint16_t mavlink_msg_camera_image_captured_get_file_url(
    const mavlink_message_t* msg, char* file_url) {
 ...
 */
static inline void mavlink_msg_camera_image_captured_decode(
    const mavlink_message_t* msg,
  ...
  mavlink_msg_camera_image_captured_get_file_url(
      msg, camera_image_captured->file_url);
#else
  uint8_t len = msg->len < MAVLINK_MSG_ID_CAMERA_IMAGE_CAPTURED_LEN
                    ? msg->len
                    : MAVLINK_MSG_ID_CAMERA_IMAGE_CAPTURED_LEN;

...
MAVPACKED(typedef struct __mavlink_protocol_version_t {
  uint16_t version; /*<  Currently active MAVLink version number * 100: v1.0 is
                       100, v2.0 is 200, etc.*/
  uint16_t min_version; /*<  Minimum MAVLink version supported*/
  uint16_t max_version; /*<  Maximum MAVLink version supported (set to the same
                           value as version by default)*/
...
#define MAVLINK_MESSAGE_INFO_PROTOCOL_VERSION                           \
  {                                                                     \
    300, "PROTOCOL_VERSION", 5, {                                       \
      {"version", NULL, MAVLINK_TYPE_UINT16_T,                          \
       0,         0,    offsetof(mavlink_protocol_version_t, version)}, \
          {"min_version",                                               \
           NULL,                                                        \
           MAVLINK_TYPE_UINT16_T,                                       \
           0,                                                           \
           2,                                                           \
           offsetof(mavlink_protocol_version_t, min_version)},          \
          {"max_version",                                               \
           NULL,                                                        \
           MAVLINK_TYPE_UINT16_T,                                       \
           0,                                                           \
           4,                                                           \
           offsetof(mavlink_protocol_version_t, max_version)},          \
          {"spec_version_hash",                                         \
           NULL,                                                        \
           MAVLINK_TYPE_UINT8_T,                                        \
           8,                                                           \
           6,                                                           \
           offsetof(mavlink_protocol_version_t, spec_version_hash)},    \
          {"library_version_hash",                                      \
           NULL,                                                        \
           MAVLINK_TYPE_UINT8_T,                                        \
           8,                                                           \
           14,                                                          \
           offsetof(mavlink_protocol_version_t, library_version_hash)}, \
    }                                                                   \
  }
#else
#define MAVLINK_MESSAGE_INFO_PROTOCOL_VERSION                           \
 ...
 *
 * @param version  Currently active MAVLink version number * 100: v1.0 is 100,
 * v2.0 is 200, etc.
 * @param min_version  Minimum MAVLink version supported
 * @param max_version  Maximum MAVLink version supported (set to the same value
 * as version by default)
 * @param spec_version_hash  The first 8 bytes (not characters printed in hex!)
 * of the git hash.
 * @param library_version_hash  The first 8 bytes (not characters printed in
 * hex!) of the git hash.
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_protocol_version_pack(
  ...
  mav_array_memcpy(packet.spec_version_hash, spec_version_hash,
                   sizeof(uint8_t) * 8);
  mav_array_memcpy(packet.library_version_hash, library_version_hash,
                   sizeof(uint8_t) * 8);
  ...
  return mavlink_finalize_message(
      msg, system_id, component_id, MAVLINK_MSG_ID_PROTOCOL_VERSION_MIN_LEN,
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_protocol_version_pack_chan(
    uint8_t system_id, uint8_t component_id, uint8_t chan,
    mavlink_message_t* msg, uint16_t version, uint16_t min_version,
    uint16_t max_version, const uint8_t* spec_version_hash,
 ...
 */
static inline uint16_t mavlink_msg_protocol_version_encode(
    uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
    const mavlink_protocol_version_t* protocol_version) {
  return mavlink_msg_protocol_version_pack(
      system_id, component_id, msg, protocol_version->version,
 ...
 * @param protocol_version C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_protocol_version_encode_chan(
    uint8_t system_id, uint8_t component_id, uint8_t chan,
    mavlink_message_t* msg,
    const mavlink_protocol_version_t* protocol_version) {
  return mavlink_msg_protocol_version_pack_chan(
      system_id, component_id, chan, msg, protocol_version->version,
      protocol_version->min_version, protocol_version->max_version,
      protocol_version->spec_version_hash,
      protocol_version->library_version_hash);
}
...
static inline void mavlink_msg_protocol_version_send(
    mavlink_channel_t chan, uint16_t version, uint16_t min_version,
    uint16_t max_version, const uint8_t* spec_version_hash,
    const uint8_t* library_version_hash) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
  char buf[MAVLINK_MSG_ID_PROTOCOL_VERSION_LEN];
  ...
  _mav_finalize_message_chan_send(
      chan, MAVLINK_MSG_ID_PROTOCOL_VERSION, (const char*)&packet,
      MAVLINK_MSG_ID_PROTOCOL_VERSION_MIN_LEN,
      MAVLINK_MSG_ID_PROTOCOL_VERSION_LEN, MAVLINK_MSG_ID_PROTOCOL_VERSION_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_protocol_version_send_struct(
    mavlink_channel_t chan,
    const mavlink_protocol_version_t* protocol_version) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
  mavlink_msg_protocol_version_send(
      chan, protocol_version->version, protocol_version->min_version,
      protocol_version->max_version, protocol_version->spec_version_hash,
      protocol_version->library_version_hash);
#else
  _mav_finalize_message_chan_send(
      chan, MAVLINK_MSG_ID_PROTOCOL_VERSION, (const char*)protocol_version,
      MAVLINK_MSG_ID_PROTOCOL_VERSION_MIN_LEN,
      MAVLINK_MSG_ID_PROTOCOL_VERSION_LEN, MAVLINK_MSG_ID_PROTOCOL_VERSION_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_protocol_version_send_buf(
    mavlink_message_t* msgbuf, mavlink_channel_t chan, uint16_t version,
 ...
 *
 * @return  Currently active MAVLink version number * 100: v1.0 is 100, v2.0 is
 * 200, etc.
 */
static inline uint16_t mavlink_msg_protocol_version_get_version(
 ...
 */
static inline uint16_t mavlink_msg_protocol_version_get_min_version(
    const mavlink_message_t* msg) {
 ...
 *
 * @return  Maximum MAVLink version supported (set to the same value as version
 * by default)
 */
static inline uint16_t mavlink_msg_protocol_version_get_max_version(
    const mavlink_message_t* msg) {
 ...
 */
static inline uint16_t mavlink_msg_protocol_version_get_spec_version_hash(
 ...
 * @return  The first 8 bytes (not characters printed in hex!) of the git hash.
 */
static inline uint16_t mavlink_msg_protocol_version_get_library_version_hash(
 ...
 */
static inline void mavlink_msg_protocol_version_decode(
    const mavlink_message_t* msg,
  ...
  mavlink_msg_protocol_version_get_spec_version_hash(
      msg, protocol_version->spec_version_hash);
  mavlink_msg_protocol_version_get_library_version_hash(
      msg, protocol_version->library_version_hash);
#else
  uint8_t len = msg->len < MAVLINK_MSG_ID_PROTOCOL_VERSION_LEN
                    ? msg->len
                    : MAVLINK_MSG_ID_PROTOCOL_VERSION_LEN;

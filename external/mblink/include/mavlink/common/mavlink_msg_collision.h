...
MAVPACKED(typedef struct __mavlink_collision_t {
  ...
  float altitude_minimum_delta; /*< [m] Closest vertical distance between
                                   vehicle and object*/
  float horizontal_minimum_delta; /*< [m] Closest horizontal distance between
                                     vehicle and object*/
  uint8_t src;                    /*<  Collision data source*/
  uint8_t action; /*<  Action that is being taken to avoid this collision*/
  uint8_t
      threat_level; /*<  How concerned the aircraft is about this collision*/
})
mavlink_collision_t;
...
#define MAVLINK_MESSAGE_INFO_COLLISION                               \
  {                                                                  \
    247, "COLLISION", 7, {                                           \
      {"src", NULL, MAVLINK_TYPE_UINT8_T,                            \
       0,     16,   offsetof(mavlink_collision_t, src)},             \
          {"id", NULL, MAVLINK_TYPE_UINT32_T,                        \
           0,    0,    offsetof(mavlink_collision_t, id)},           \
          {"action", NULL, MAVLINK_TYPE_UINT8_T,                     \
           0,        17,   offsetof(mavlink_collision_t, action)},   \
          {"threat_level",                                           \
           NULL,                                                     \
           MAVLINK_TYPE_UINT8_T,                                     \
           0,                                                        \
           18,                                                       \
           offsetof(mavlink_collision_t, threat_level)},             \
          {"time_to_minimum_delta",                                  \
           NULL,                                                     \
           MAVLINK_TYPE_FLOAT,                                       \
           0,                                                        \
           4,                                                        \
           offsetof(mavlink_collision_t, time_to_minimum_delta)},    \
          {"altitude_minimum_delta",                                 \
           NULL,                                                     \
           MAVLINK_TYPE_FLOAT,                                       \
           0,                                                        \
           8,                                                        \
           offsetof(mavlink_collision_t, altitude_minimum_delta)},   \
          {"horizontal_minimum_delta",                               \
           NULL,                                                     \
           MAVLINK_TYPE_FLOAT,                                       \
           0,                                                        \
           12,                                                       \
           offsetof(mavlink_collision_t, horizontal_minimum_delta)}, \
    }                                                                \
  }
#else
#define MAVLINK_MESSAGE_INFO_COLLISION                               \
 ...
 * @param altitude_minimum_delta [m] Closest vertical distance between vehicle
 * and object
 * @param horizontal_minimum_delta [m] Closest horizontal distance between
 * vehicle and object
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_collision_pack(
  ...
  return mavlink_finalize_message(
      msg, system_id, component_id, MAVLINK_MSG_ID_COLLISION_MIN_LEN,
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_collision_pack_chan(
    uint8_t system_id, uint8_t component_id, uint8_t chan,
    mavlink_message_t* msg, uint8_t src, uint32_t id, uint8_t action,
    uint8_t threat_level, float time_to_minimum_delta,
    float altitude_minimum_delta, float horizontal_minimum_delta) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
  char buf[MAVLINK_MSG_ID_COLLISION_LEN];
  _mav_put_uint32_t(buf, 0, id);
  _mav_put_float(buf, 4, time_to_minimum_delta);
  ...
  return mavlink_finalize_message_chan(
      msg, system_id, component_id, chan, MAVLINK_MSG_ID_COLLISION_MIN_LEN,
      MAVLINK_MSG_ID_COLLISION_LEN, MAVLINK_MSG_ID_COLLISION_CRC);
}
 ...
 */
static inline uint16_t mavlink_msg_collision_encode(
    uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
    const mavlink_collision_t* collision) {
  return mavlink_msg_collision_pack(
      system_id, component_id, msg, collision->src, collision->id,
      collision->action, collision->threat_level,
      collision->time_to_minimum_delta, collision->altitude_minimum_delta,
 ...
 * @param collision C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_collision_encode_chan(
    uint8_t system_id, uint8_t component_id, uint8_t chan,
    mavlink_message_t* msg, const mavlink_collision_t* collision) {
  return mavlink_msg_collision_pack_chan(
      system_id, component_id, chan, msg, collision->src, collision->id,
      collision->action, collision->threat_level,
      collision->time_to_minimum_delta, collision->altitude_minimum_delta,
      collision->horizontal_minimum_delta);
}
...
static inline void mavlink_msg_collision_send(
    mavlink_channel_t chan, uint8_t src, uint32_t id, uint8_t action,
    uint8_t threat_level, float time_to_minimum_delta,
    float altitude_minimum_delta, float horizontal_minimum_delta) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
  char buf[MAVLINK_MSG_ID_COLLISION_LEN];
  ...
  _mav_finalize_message_chan_send(
      chan, MAVLINK_MSG_ID_COLLISION, buf, MAVLINK_MSG_ID_COLLISION_MIN_LEN,
      MAVLINK_MSG_ID_COLLISION_LEN, MAVLINK_MSG_ID_COLLISION_CRC);
#else
  mavlink_collision_t packet;
  packet.id = id;
  ...
  _mav_finalize_message_chan_send(
      chan, MAVLINK_MSG_ID_COLLISION, (const char*)&packet,
      MAVLINK_MSG_ID_COLLISION_MIN_LEN, MAVLINK_MSG_ID_COLLISION_LEN,
 ...
 */
static inline void mavlink_msg_collision_send_struct(
    mavlink_channel_t chan, const mavlink_collision_t* collision) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
  mavlink_msg_collision_send(
      chan, collision->src, collision->id, collision->action,
      collision->threat_level, collision->time_to_minimum_delta,
      collision->altitude_minimum_delta, collision->horizontal_minimum_delta);
#else
  _mav_finalize_message_chan_send(
      chan, MAVLINK_MSG_ID_COLLISION, (const char*)collision,
      MAVLINK_MSG_ID_COLLISION_MIN_LEN, MAVLINK_MSG_ID_COLLISION_LEN,
      MAVLINK_MSG_ID_COLLISION_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_collision_send_buf(
    mavlink_message_t* msgbuf, mavlink_channel_t chan, uint8_t src, uint32_t id,
    uint8_t action, uint8_t threat_level, float time_to_minimum_delta,
    float altitude_minimum_delta, float horizontal_minimum_delta) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
 ...
 */
static inline uint8_t mavlink_msg_collision_get_src(
 ...
 */
static inline uint32_t mavlink_msg_collision_get_id(
    const mavlink_message_t* msg) {
 ...
 */
static inline uint8_t mavlink_msg_collision_get_action(
    const mavlink_message_t* msg) {
 ...
 */
static inline uint8_t mavlink_msg_collision_get_threat_level(
    const mavlink_message_t* msg) {
 ...
 */
static inline float mavlink_msg_collision_get_time_to_minimum_delta(
    const mavlink_message_t* msg) {
 ...
 */
static inline float mavlink_msg_collision_get_altitude_minimum_delta(
    const mavlink_message_t* msg) {
 ...
 */
static inline float mavlink_msg_collision_get_horizontal_minimum_delta(
    const mavlink_message_t* msg) {
 ...
 */
static inline void mavlink_msg_collision_decode(
    const mavlink_message_t* msg, mavlink_collision_t* collision) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
...
#else
  uint8_t len = msg->len < MAVLINK_MSG_ID_COLLISION_LEN
                    ? msg->len

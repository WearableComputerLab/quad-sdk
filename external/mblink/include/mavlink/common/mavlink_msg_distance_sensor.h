...
MAVPACKED(typedef struct __mavlink_distance_sensor_t {
  ...
  uint8_t orientation;       /*<  Direction the sensor faces. downward-facing:
  ...
  uint8_t covariance; /*< [cm^2] Measurement variance. Max standard deviation is
                         6cm. 255 if unknown.*/
  ...
  float vertical_fov;   /*< [rad] Vertical Field of View (angle) where the
                           distance measurement is valid and the field of view is
                           known. Otherwise this is set to 0.*/
  float quaternion[4]; /*<  Quaternion of the sensor orientation in vehicle body
                          frame (w, x, y, z order, zero-rotation is 1, 0, 0, 0).
                          Zero-rotation is along the vehicle body x-axis. This
                          field is required if the orientation is set to
...
#define MAVLINK_MESSAGE_INFO_DISTANCE_SENSOR                         \
  {                                                                  \
    132, "DISTANCE_SENSOR", 11, {                                    \
      {"time_boot_ms",                                               \
       NULL,                                                         \
       MAVLINK_TYPE_UINT32_T,                                        \
       0,                                                            \
       0,                                                            \
       offsetof(mavlink_distance_sensor_t, time_boot_ms)},           \
          {"min_distance",                                           \
           NULL,                                                     \
           MAVLINK_TYPE_UINT16_T,                                    \
           0,                                                        \
           4,                                                        \
           offsetof(mavlink_distance_sensor_t, min_distance)},       \
          {"max_distance",                                           \
           NULL,                                                     \
           MAVLINK_TYPE_UINT16_T,                                    \
           0,                                                        \
           6,                                                        \
           offsetof(mavlink_distance_sensor_t, max_distance)},       \
          {"current_distance",                                       \
           NULL,                                                     \
           MAVLINK_TYPE_UINT16_T,                                    \
           0,                                                        \
           8,                                                        \
           offsetof(mavlink_distance_sensor_t, current_distance)},   \
          {"type", NULL, MAVLINK_TYPE_UINT8_T,                       \
           0,      10,   offsetof(mavlink_distance_sensor_t, type)}, \
          {"id", NULL, MAVLINK_TYPE_UINT8_T,                         \
           0,    11,   offsetof(mavlink_distance_sensor_t, id)},     \
          {"orientation",                                            \
           NULL,                                                     \
           MAVLINK_TYPE_UINT8_T,                                     \
           0,                                                        \
           12,                                                       \
           offsetof(mavlink_distance_sensor_t, orientation)},        \
          {"covariance",                                             \
           NULL,                                                     \
           MAVLINK_TYPE_UINT8_T,                                     \
           0,                                                        \
           13,                                                       \
           offsetof(mavlink_distance_sensor_t, covariance)},         \
          {"horizontal_fov",                                         \
           NULL,                                                     \
           MAVLINK_TYPE_FLOAT,                                       \
           0,                                                        \
           14,                                                       \
           offsetof(mavlink_distance_sensor_t, horizontal_fov)},     \
          {"vertical_fov",                                           \
           NULL,                                                     \
           MAVLINK_TYPE_FLOAT,                                       \
           0,                                                        \
           18,                                                       \
           offsetof(mavlink_distance_sensor_t, vertical_fov)},       \
          {"quaternion",                                             \
           NULL,                                                     \
           MAVLINK_TYPE_FLOAT,                                       \
           4,                                                        \
           22,                                                       \
           offsetof(mavlink_distance_sensor_t, quaternion)},         \
    }                                                                \
  }
#else
#define MAVLINK_MESSAGE_INFO_DISTANCE_SENSOR                         \
 ...
 * @param orientation  Direction the sensor faces. downward-facing:
 ...
 * distance measurement is valid and the field of view is known. Otherwise this
 * is set to 0.
 * @param vertical_fov [rad] Vertical Field of View (angle) where the distance
 * measurement is valid and the field of view is known. Otherwise this is set to
 * 0.
 * @param quaternion  Quaternion of the sensor orientation in vehicle body frame
 * (w, x, y, z order, zero-rotation is 1, 0, 0, 0). Zero-rotation is along the
...
static inline uint16_t mavlink_msg_distance_sensor_pack(
    ...
    uint32_t time_boot_ms, uint16_t min_distance, uint16_t max_distance,
    uint16_t current_distance, uint8_t type, uint8_t id, uint8_t orientation,
  ...
  return mavlink_finalize_message(
      msg, system_id, component_id, MAVLINK_MSG_ID_DISTANCE_SENSOR_MIN_LEN,
      MAVLINK_MSG_ID_DISTANCE_SENSOR_LEN, MAVLINK_MSG_ID_DISTANCE_SENSOR_CRC);
}
...
static inline uint16_t mavlink_msg_distance_sensor_pack_chan(
    ...
    mavlink_message_t* msg, uint32_t time_boot_ms, uint16_t min_distance,
    uint16_t max_distance, uint16_t current_distance, uint8_t type, uint8_t id,
    uint8_t orientation, uint8_t covariance, float horizontal_fov,
    float vertical_fov, const float* quaternion) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
  char buf[MAVLINK_MSG_ID_DISTANCE_SENSOR_LEN];
  _mav_put_uint32_t(buf, 0, time_boot_ms);
  _mav_put_uint16_t(buf, 4, min_distance);
...
static inline uint16_t mavlink_msg_distance_sensor_encode(
  ...
  return mavlink_msg_distance_sensor_pack(
      system_id, component_id, msg, distance_sensor->time_boot_ms,
      distance_sensor->min_distance, distance_sensor->max_distance,
      distance_sensor->current_distance, distance_sensor->type,
      distance_sensor->id, distance_sensor->orientation,
      distance_sensor->covariance, distance_sensor->horizontal_fov,
      distance_sensor->vertical_fov, distance_sensor->quaternion);
}
...
static inline uint16_t mavlink_msg_distance_sensor_encode_chan(
    uint8_t system_id, uint8_t component_id, uint8_t chan,
    mavlink_message_t* msg, const mavlink_distance_sensor_t* distance_sensor) {
  return mavlink_msg_distance_sensor_pack_chan(
      system_id, component_id, chan, msg, distance_sensor->time_boot_ms,
      distance_sensor->min_distance, distance_sensor->max_distance,
      distance_sensor->current_distance, distance_sensor->type,
      distance_sensor->id, distance_sensor->orientation,
      distance_sensor->covariance, distance_sensor->horizontal_fov,
...
static inline void mavlink_msg_distance_sensor_send(
    mavlink_channel_t chan, uint32_t time_boot_ms, uint16_t min_distance,
    uint16_t max_distance, uint16_t current_distance, uint8_t type, uint8_t id,
    uint8_t orientation, uint8_t covariance, float horizontal_fov,
    float vertical_fov, const float* quaternion) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
  ...
  _mav_put_float(buf, 18, vertical_fov);
  _mav_put_float_array(buf, 22, quaternion, 4);
  _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_DISTANCE_SENSOR, buf,
                                  MAVLINK_MSG_ID_DISTANCE_SENSOR_MIN_LEN,
                                  MAVLINK_MSG_ID_DISTANCE_SENSOR_LEN,
                                  MAVLINK_MSG_ID_DISTANCE_SENSOR_CRC);
#else
  ...
  _mav_finalize_message_chan_send(
      chan, MAVLINK_MSG_ID_DISTANCE_SENSOR, (const char*)&packet,
      MAVLINK_MSG_ID_DISTANCE_SENSOR_MIN_LEN,
      MAVLINK_MSG_ID_DISTANCE_SENSOR_LEN, MAVLINK_MSG_ID_DISTANCE_SENSOR_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_distance_sensor_send_struct(
    mavlink_channel_t chan, const mavlink_distance_sensor_t* distance_sensor) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
  mavlink_msg_distance_sensor_send(
      chan, distance_sensor->time_boot_ms, distance_sensor->min_distance,
      distance_sensor->max_distance, distance_sensor->current_distance,
      distance_sensor->type, distance_sensor->id, distance_sensor->orientation,
      distance_sensor->covariance, distance_sensor->horizontal_fov,
      distance_sensor->vertical_fov, distance_sensor->quaternion);
#else
  _mav_finalize_message_chan_send(
      chan, MAVLINK_MSG_ID_DISTANCE_SENSOR, (const char*)distance_sensor,
      MAVLINK_MSG_ID_DISTANCE_SENSOR_MIN_LEN,
      MAVLINK_MSG_ID_DISTANCE_SENSOR_LEN, MAVLINK_MSG_ID_DISTANCE_SENSOR_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_distance_sensor_send_buf(
    mavlink_message_t* msgbuf, mavlink_channel_t chan, uint32_t time_boot_ms,
    uint16_t min_distance, uint16_t max_distance, uint16_t current_distance,
    uint8_t type, uint8_t id, uint8_t orientation, uint8_t covariance,
    float horizontal_fov, float vertical_fov, const float* quaternion) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
 ...
 */
static inline uint32_t mavlink_msg_distance_sensor_get_time_boot_ms(
 ...
 */
static inline uint16_t mavlink_msg_distance_sensor_get_min_distance(
    const mavlink_message_t* msg) {
 ...
 */
static inline uint16_t mavlink_msg_distance_sensor_get_max_distance(
    const mavlink_message_t* msg) {
 ...
 */
static inline uint16_t mavlink_msg_distance_sensor_get_current_distance(
    const mavlink_message_t* msg) {
 ...
 */
static inline uint8_t mavlink_msg_distance_sensor_get_type(
    const mavlink_message_t* msg) {
 ...
 */
static inline uint8_t mavlink_msg_distance_sensor_get_id(
    const mavlink_message_t* msg) {
 ...
 *
 * @return  Direction the sensor faces. downward-facing: ROTATION_PITCH_270,
 ...
 * ROTATION_YAW_270
 */
static inline uint8_t mavlink_msg_distance_sensor_get_orientation(
    const mavlink_message_t* msg) {
 ...
 *
 * @return [cm^2] Measurement variance. Max standard deviation is 6cm. 255 if
 * unknown.
 */
static inline uint8_t mavlink_msg_distance_sensor_get_covariance(
    const mavlink_message_t* msg) {
 ...
 *
 * @return [rad] Horizontal Field of View (angle) where the distance measurement
 * is valid and the field of view is known. Otherwise this is set to 0.
 */
static inline float mavlink_msg_distance_sensor_get_horizontal_fov(
    const mavlink_message_t* msg) {
 ...
 *
 * @return [rad] Vertical Field of View (angle) where the distance measurement
 * is valid and the field of view is known. Otherwise this is set to 0.
 */
static inline float mavlink_msg_distance_sensor_get_vertical_fov(
    const mavlink_message_t* msg) {
 ...
 *
 * @return  Quaternion of the sensor orientation in vehicle body frame (w, x, y,
 * z order, zero-rotation is 1, 0, 0, 0). Zero-rotation is along the vehicle
...
static inline uint16_t mavlink_msg_distance_sensor_get_quaternion(
    const mavlink_message_t* msg, float* quaternion) {
 ...
 */
static inline void mavlink_msg_distance_sensor_decode(
  ...
  distance_sensor->horizontal_fov =
      mavlink_msg_distance_sensor_get_horizontal_fov(msg);
  distance_sensor->vertical_fov =
      mavlink_msg_distance_sensor_get_vertical_fov(msg);
  mavlink_msg_distance_sensor_get_quaternion(msg, distance_sensor->quaternion);
#else
  uint8_t len = msg->len < MAVLINK_MSG_ID_DISTANCE_SENSOR_LEN
                    ? msg->len
                    : MAVLINK_MSG_ID_DISTANCE_SENSOR_LEN;

...
MAVPACKED(typedef struct __mavlink_odometry_t {
  uint64_t time_usec; /*< [us] Timestamp (UNIX Epoch time or time since system
  ...
  float velocity_covariance
      [21]; /*<  Row-major representation of a 6x6 velocity cross-covariance
               ...
               pitchspeed, yawspeed; first six entries are the first ROW, next
               five entries are the second ROW, etc.). If unknown, assign NaN
               value to first element in the array.*/
  uint8_t frame_id; /*<  Coordinate frame of reference for the pose data.*/
  uint8_t child_frame_id; /*<  Coordinate frame of reference for the velocity in
                             free space (twist) data.*/
  uint8_t
      reset_counter; /*<  Estimate reset counter. This should be incremented
                        when the estimate resets in any of the dimensions
                        (position, velocity, attitude, angular speed). This is
                        designed to be used when e.g an external SLAM system
                        detects a loop-closure and the estimate jumps.*/
})
mavlink_odometry_t;
...
#define MAVLINK_MESSAGE_INFO_ODOMETRY                                   \
  {                                                                     \
    331, "ODOMETRY", 16, {                                              \
      {"time_usec", NULL, MAVLINK_TYPE_UINT64_T,                        \
       0,           0,    offsetof(mavlink_odometry_t, time_usec)},     \
          {"frame_id", NULL, MAVLINK_TYPE_UINT8_T,                      \
           0,          228,  offsetof(mavlink_odometry_t, frame_id)},   \
          {"child_frame_id",                                            \
           NULL,                                                        \
           MAVLINK_TYPE_UINT8_T,                                        \
           0,                                                           \
           229,                                                         \
           offsetof(mavlink_odometry_t, child_frame_id)},               \
          {"x", NULL, MAVLINK_TYPE_FLOAT,                               \
           0,   8,    offsetof(mavlink_odometry_t, x)},                 \
          {"y", NULL, MAVLINK_TYPE_FLOAT,                               \
           0,   12,   offsetof(mavlink_odometry_t, y)},                 \
          {"z", NULL, MAVLINK_TYPE_FLOAT,                               \
           0,   16,   offsetof(mavlink_odometry_t, z)},                 \
          {"q", NULL, MAVLINK_TYPE_FLOAT,                               \
           4,   20,   offsetof(mavlink_odometry_t, q)},                 \
          {"vx", NULL, MAVLINK_TYPE_FLOAT,                              \
           0,    36,   offsetof(mavlink_odometry_t, vx)},               \
          {"vy", NULL, MAVLINK_TYPE_FLOAT,                              \
           0,    40,   offsetof(mavlink_odometry_t, vy)},               \
          {"vz", NULL, MAVLINK_TYPE_FLOAT,                              \
           0,    44,   offsetof(mavlink_odometry_t, vz)},               \
          {"rollspeed", NULL, MAVLINK_TYPE_FLOAT,                       \
           0,           48,   offsetof(mavlink_odometry_t, rollspeed)}, \
          {"pitchspeed",                                                \
           NULL,                                                        \
           MAVLINK_TYPE_FLOAT,                                          \
           0,                                                           \
           52,                                                          \
           offsetof(mavlink_odometry_t, pitchspeed)},                   \
          {"yawspeed", NULL, MAVLINK_TYPE_FLOAT,                        \
           0,          56,   offsetof(mavlink_odometry_t, yawspeed)},   \
          {"pose_covariance",                                           \
           NULL,                                                        \
           MAVLINK_TYPE_FLOAT,                                          \
           21,                                                          \
           60,                                                          \
           offsetof(mavlink_odometry_t, pose_covariance)},              \
          {"velocity_covariance",                                       \
           NULL,                                                        \
           MAVLINK_TYPE_FLOAT,                                          \
           21,                                                          \
           144,                                                         \
           offsetof(mavlink_odometry_t, velocity_covariance)},          \
          {"reset_counter",                                             \
           NULL,                                                        \
           MAVLINK_TYPE_UINT8_T,                                        \
           0,                                                           \
           230,                                                         \
           offsetof(mavlink_odometry_t, reset_counter)},                \
    }                                                                   \
  }
#else
#define MAVLINK_MESSAGE_INFO_ODOMETRY                                   \
 ...
 * @param child_frame_id  Coordinate frame of reference for the velocity in free
 * space (twist) data.
 ...
 * cross-covariance matrix upper right triangle (states: x, y, z, roll, pitch,
 * yaw; first six entries are the first ROW, next five entries are the second
 ...
 * cross-covariance matrix upper right triangle (states: vx, vy, vz, rollspeed,
 * pitchspeed, yawspeed; first six entries are the first ROW, next five entries
 * are the second ROW, etc.). If unknown, assign NaN value to first element in
 * the array.
 * @param reset_counter  Estimate reset counter. This should be incremented when
 * the estimate resets in any of the dimensions (position, velocity, attitude,
 * angular speed). This is designed to be used when e.g an external SLAM system
 * detects a loop-closure and the estimate jumps.
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_odometry_pack(
    ...
    const float* pose_covariance, const float* velocity_covariance,
  ...
  mav_array_memcpy(packet.velocity_covariance, velocity_covariance,
                   sizeof(float) * 21);
  ...
  return mavlink_finalize_message(
      msg, system_id, component_id, MAVLINK_MSG_ID_ODOMETRY_MIN_LEN,
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_odometry_pack_chan(
    uint8_t system_id, uint8_t component_id, uint8_t chan,
    mavlink_message_t* msg, uint64_t time_usec, uint8_t frame_id,
    uint8_t child_frame_id, float x, float y, float z, const float* q, float vx,
  ...
  return mavlink_finalize_message_chan(
      msg, system_id, component_id, chan, MAVLINK_MSG_ID_ODOMETRY_MIN_LEN,
      MAVLINK_MSG_ID_ODOMETRY_LEN, MAVLINK_MSG_ID_ODOMETRY_CRC);
}
 ...
 */
static inline uint16_t mavlink_msg_odometry_encode(
    uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
  ...
  return mavlink_msg_odometry_pack(
      ...
      odometry->child_frame_id, odometry->x, odometry->y, odometry->z,
      odometry->q, odometry->vx, odometry->vy, odometry->vz,
      odometry->rollspeed, odometry->pitchspeed, odometry->yawspeed,
      odometry->pose_covariance, odometry->velocity_covariance,
 ...
 * @param odometry C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_odometry_encode_chan(
    uint8_t system_id, uint8_t component_id, uint8_t chan,
    mavlink_message_t* msg, const mavlink_odometry_t* odometry) {
  return mavlink_msg_odometry_pack_chan(
      system_id, component_id, chan, msg, odometry->time_usec,
      odometry->frame_id, odometry->child_frame_id, odometry->x, odometry->y,
      odometry->z, odometry->q, odometry->vx, odometry->vy, odometry->vz,
      odometry->rollspeed, odometry->pitchspeed, odometry->yawspeed,
      odometry->pose_covariance, odometry->velocity_covariance,
      odometry->reset_counter);
}
...
static inline void mavlink_msg_odometry_send(
  ...
  _mav_finalize_message_chan_send(
      chan, MAVLINK_MSG_ID_ODOMETRY, buf, MAVLINK_MSG_ID_ODOMETRY_MIN_LEN,
      MAVLINK_MSG_ID_ODOMETRY_LEN, MAVLINK_MSG_ID_ODOMETRY_CRC);
#else
  mavlink_odometry_t packet;
  packet.time_usec = time_usec;
  ...
  mav_array_memcpy(packet.velocity_covariance, velocity_covariance,
                   sizeof(float) * 21);
  _mav_finalize_message_chan_send(
      chan, MAVLINK_MSG_ID_ODOMETRY, (const char*)&packet,
      MAVLINK_MSG_ID_ODOMETRY_MIN_LEN, MAVLINK_MSG_ID_ODOMETRY_LEN,
 ...
 */
static inline void mavlink_msg_odometry_send_struct(
    mavlink_channel_t chan, const mavlink_odometry_t* odometry) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
  mavlink_msg_odometry_send(
      chan, odometry->time_usec, odometry->frame_id, odometry->child_frame_id,
      odometry->x, odometry->y, odometry->z, odometry->q, odometry->vx,
      odometry->vy, odometry->vz, odometry->rollspeed, odometry->pitchspeed,
      odometry->yawspeed, odometry->pose_covariance,
      odometry->velocity_covariance, odometry->reset_counter);
#else
  _mav_finalize_message_chan_send(
      chan, MAVLINK_MSG_ID_ODOMETRY, (const char*)odometry,
      MAVLINK_MSG_ID_ODOMETRY_MIN_LEN, MAVLINK_MSG_ID_ODOMETRY_LEN,
      MAVLINK_MSG_ID_ODOMETRY_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_odometry_send_buf(
    mavlink_message_t* msgbuf, mavlink_channel_t chan, uint64_t time_usec,
    uint8_t frame_id, uint8_t child_frame_id, float x, float y, float z,
    const float* q, float vx, float vy, float vz, float rollspeed,
    float pitchspeed, float yawspeed, const float* pose_covariance,
    const float* velocity_covariance, uint8_t reset_counter) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
  ...
  packet->reset_counter = reset_counter;
  mav_array_memcpy(packet->q, q, sizeof(float) * 4);
  mav_array_memcpy(packet->pose_covariance, pose_covariance,
                   sizeof(float) * 21);
  mav_array_memcpy(packet->velocity_covariance, velocity_covariance,
                   sizeof(float) * 21);
  _mav_finalize_message_chan_send(
 ...
 *
 * @return [us] Timestamp (UNIX Epoch time or time since system boot). The
 * receiving end can infer timestamp format (since 1.1.1970 or since system
 * boot) by checking for the magnitude the number.
 */
static inline uint64_t mavlink_msg_odometry_get_time_usec(
 ...
 */
static inline uint8_t mavlink_msg_odometry_get_frame_id(
    const mavlink_message_t* msg) {
 ...
 *
 * @return  Coordinate frame of reference for the velocity in free space (twist)
 * data.
 */
static inline uint8_t mavlink_msg_odometry_get_child_frame_id(
    const mavlink_message_t* msg) {
 ...
 */
static inline uint16_t mavlink_msg_odometry_get_q(const mavlink_message_t* msg,
                                                  float* q) {
 ...
 */
static inline float mavlink_msg_odometry_get_rollspeed(
    const mavlink_message_t* msg) {
 ...
 */
static inline float mavlink_msg_odometry_get_pitchspeed(
    const mavlink_message_t* msg) {
 ...
 */
static inline float mavlink_msg_odometry_get_yawspeed(
    const mavlink_message_t* msg) {
 ...
 *
 * @return  Row-major representation of a 6x6 pose cross-covariance matrix upper
 * right triangle (states: x, y, z, roll, pitch, yaw; first six entries are the
 * first ROW, next five entries are the second ROW, etc.). If unknown, assign
 * NaN value to first element in the array.
 */
static inline uint16_t mavlink_msg_odometry_get_pose_covariance(
    const mavlink_message_t* msg, float* pose_covariance) {
 ...
 *
 * @return  Row-major representation of a 6x6 velocity cross-covariance matrix
 * upper right triangle (states: vx, vy, vz, rollspeed, pitchspeed, yawspeed;
 * first six entries are the first ROW, next five entries are the second ROW,
 * etc.). If unknown, assign NaN value to first element in the array.
 */
static inline uint16_t mavlink_msg_odometry_get_velocity_covariance(
    const mavlink_message_t* msg, float* velocity_covariance) {
 ...
 *
 * @return  Estimate reset counter. This should be incremented when the estimate
 * resets in any of the dimensions (position, velocity, attitude, angular
 * speed). This is designed to be used when e.g an external SLAM system detects
 * a loop-closure and the estimate jumps.
 */
static inline uint8_t mavlink_msg_odometry_get_reset_counter(
    const mavlink_message_t* msg) {
 ...
 */
static inline void mavlink_msg_odometry_decode(const mavlink_message_t* msg,
  ...
  mavlink_msg_odometry_get_velocity_covariance(msg,
...
#else
  uint8_t len = msg->len < MAVLINK_MSG_ID_ODOMETRY_LEN
                    ? msg->len

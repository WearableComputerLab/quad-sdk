...
MAVPACKED(typedef struct __mavlink_follow_target_t {
...
#define MAVLINK_MESSAGE_INFO_FOLLOW_TARGET                               \
  {                                                                      \
    144, "FOLLOW_TARGET", 11, {                                          \
      {"timestamp", NULL, MAVLINK_TYPE_UINT64_T,                         \
       0,           0,    offsetof(mavlink_follow_target_t, timestamp)}, \
          {"est_capabilities",                                           \
           NULL,                                                         \
           MAVLINK_TYPE_UINT8_T,                                         \
           0,                                                            \
           92,                                                           \
           offsetof(mavlink_follow_target_t, est_capabilities)},         \
          {"lat", NULL, MAVLINK_TYPE_INT32_T,                            \
           0,     16,   offsetof(mavlink_follow_target_t, lat)},         \
          {"lon", NULL, MAVLINK_TYPE_INT32_T,                            \
           0,     20,   offsetof(mavlink_follow_target_t, lon)},         \
          {"alt", NULL, MAVLINK_TYPE_FLOAT,                              \
           0,     24,   offsetof(mavlink_follow_target_t, alt)},         \
          {"vel", NULL, MAVLINK_TYPE_FLOAT,                              \
           3,     28,   offsetof(mavlink_follow_target_t, vel)},         \
          {"acc", NULL, MAVLINK_TYPE_FLOAT,                              \
           3,     40,   offsetof(mavlink_follow_target_t, acc)},         \
          {"attitude_q",                                                 \
           NULL,                                                         \
           MAVLINK_TYPE_FLOAT,                                           \
           4,                                                            \
           52,                                                           \
           offsetof(mavlink_follow_target_t, attitude_q)},               \
          {"rates", NULL, MAVLINK_TYPE_FLOAT,                            \
           3,       68,   offsetof(mavlink_follow_target_t, rates)},     \
          {"position_cov",                                               \
           NULL,                                                         \
           MAVLINK_TYPE_FLOAT,                                           \
           3,                                                            \
           80,                                                           \
           offsetof(mavlink_follow_target_t, position_cov)},             \
          {"custom_state",                                               \
           NULL,                                                         \
           MAVLINK_TYPE_UINT64_T,                                        \
           0,                                                            \
           8,                                                            \
           offsetof(mavlink_follow_target_t, custom_state)},             \
    }                                                                    \
  }
#else
#define MAVLINK_MESSAGE_INFO_FOLLOW_TARGET                               \
 ...
 * @param est_capabilities  bit positions for tracker reporting capabilities
 * (POS = 0, VEL = 1, ACCEL = 2, ATT + RATES = 3)
 ...
 */
static inline uint16_t mavlink_msg_follow_target_pack(
    uint8_t system_id, uint8_t component_id, mavlink_message_t *msg,
    uint64_t timestamp, uint8_t est_capabilities, int32_t lat, int32_t lon,
    float alt, const float *vel, const float *acc, const float *attitude_q,
    const float *rates, const float *position_cov, uint64_t custom_state) {
  ...
  return mavlink_finalize_message(
      ...
      MAVLINK_MSG_ID_FOLLOW_TARGET_LEN, MAVLINK_MSG_ID_FOLLOW_TARGET_CRC);
}
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_follow_target_pack_chan(
    uint8_t system_id, uint8_t component_id, uint8_t chan,
    mavlink_message_t *msg, uint64_t timestamp, uint8_t est_capabilities,
    int32_t lat, int32_t lon, float alt, const float *vel, const float *acc,
    const float *attitude_q, const float *rates, const float *position_cov,
 ...
 */
static inline uint16_t mavlink_msg_follow_target_encode(
    uint8_t system_id, uint8_t component_id, mavlink_message_t *msg,
    const mavlink_follow_target_t *follow_target) {
  return mavlink_msg_follow_target_pack(
      system_id, component_id, msg, follow_target->timestamp,
      follow_target->est_capabilities, follow_target->lat, follow_target->lon,
      follow_target->alt, follow_target->vel, follow_target->acc,
 ...
 * @param follow_target C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_follow_target_encode_chan(
    uint8_t system_id, uint8_t component_id, uint8_t chan,
    mavlink_message_t *msg, const mavlink_follow_target_t *follow_target) {
  return mavlink_msg_follow_target_pack_chan(
      system_id, component_id, chan, msg, follow_target->timestamp,
      follow_target->est_capabilities, follow_target->lat, follow_target->lon,
      follow_target->alt, follow_target->vel, follow_target->acc,
      follow_target->attitude_q, follow_target->rates,
      follow_target->position_cov, follow_target->custom_state);
...
static inline void mavlink_msg_follow_target_send(
    mavlink_channel_t chan, uint64_t timestamp, uint8_t est_capabilities,
    int32_t lat, int32_t lon, float alt, const float *vel, const float *acc,
    const float *attitude_q, const float *rates, const float *position_cov,
    uint64_t custom_state) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
  ...
  _mav_put_float_array(buf, 68, rates, 3);
  _mav_put_float_array(buf, 80, position_cov, 3);
  _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_FOLLOW_TARGET, buf,
  ...
  _mav_finalize_message_chan_send(
      chan, MAVLINK_MSG_ID_FOLLOW_TARGET, (const char *)&packet,
      MAVLINK_MSG_ID_FOLLOW_TARGET_MIN_LEN, MAVLINK_MSG_ID_FOLLOW_TARGET_LEN,
      MAVLINK_MSG_ID_FOLLOW_TARGET_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_follow_target_send_struct(
    mavlink_channel_t chan, const mavlink_follow_target_t *follow_target) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
  mavlink_msg_follow_target_send(
      chan, follow_target->timestamp, follow_target->est_capabilities,
      follow_target->lat, follow_target->lon, follow_target->alt,
      follow_target->vel, follow_target->acc, follow_target->attitude_q,
      follow_target->rates, follow_target->position_cov,
      follow_target->custom_state);
#else
  _mav_finalize_message_chan_send(
      chan, MAVLINK_MSG_ID_FOLLOW_TARGET, (const char *)follow_target,
      MAVLINK_MSG_ID_FOLLOW_TARGET_MIN_LEN, MAVLINK_MSG_ID_FOLLOW_TARGET_LEN,
      MAVLINK_MSG_ID_FOLLOW_TARGET_CRC);
#endif
}
...
static inline void mavlink_msg_follow_target_send_buf(
    ...
    uint8_t est_capabilities, int32_t lat, int32_t lon, float alt,
    const float *vel, const float *acc, const float *attitude_q,
    const float *rates, const float *position_cov, uint64_t custom_state) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
 ...
 */
static inline uint64_t mavlink_msg_follow_target_get_timestamp(
 ...
 *
 * @return  bit positions for tracker reporting capabilities (POS = 0, VEL = 1,
 * ACCEL = 2, ATT + RATES = 3)
 */
static inline uint8_t mavlink_msg_follow_target_get_est_capabilities(
    const mavlink_message_t *msg) {
 ...
 */
static inline int32_t mavlink_msg_follow_target_get_lat(
    const mavlink_message_t *msg) {
 ...
 */
static inline int32_t mavlink_msg_follow_target_get_lon(
    const mavlink_message_t *msg) {
 ...
 */
static inline float mavlink_msg_follow_target_get_alt(
    const mavlink_message_t *msg) {
 ...
 */
static inline uint16_t mavlink_msg_follow_target_get_vel(
    const mavlink_message_t *msg, float *vel) {
 ...
 */
static inline uint16_t mavlink_msg_follow_target_get_acc(
    const mavlink_message_t *msg, float *acc) {
 ...
 */
static inline uint16_t mavlink_msg_follow_target_get_attitude_q(
    const mavlink_message_t *msg, float *attitude_q) {
 ...
 */
static inline uint16_t mavlink_msg_follow_target_get_rates(
    const mavlink_message_t *msg, float *rates) {
 ...
 * @return  eph epv
 */
static inline uint16_t mavlink_msg_follow_target_get_position_cov(
    const mavlink_message_t *msg, float *position_cov) {
 ...
 */
static inline uint64_t mavlink_msg_follow_target_get_custom_state(
    const mavlink_message_t *msg) {
 ...
 */
static inline void mavlink_msg_follow_target_decode(
    const mavlink_message_t *msg, mavlink_follow_target_t *follow_target) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
  ...
  follow_target->est_capabilities =
      mavlink_msg_follow_target_get_est_capabilities(msg);
#else
  uint8_t len = msg->len < MAVLINK_MSG_ID_FOLLOW_TARGET_LEN
                    ? msg->len
                    : MAVLINK_MSG_ID_FOLLOW_TARGET_LEN;

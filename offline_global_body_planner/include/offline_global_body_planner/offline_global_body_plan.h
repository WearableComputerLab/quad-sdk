...
class OfflineGlobalBodyPlan {
   public:
     ...
     */
    void getNominalHeight(State &s, const PlannerConfig &planner_config);
    ...
    /**
     * @brief Load plan by adding additional parameters to be viable for quad-sdk framework
     * 
     */
    void loadPlanData(double t0, double dt, const FullState &start_state, 
                    std::vector<State> &state_sequence, std::vector<GRF> &grf_plan,

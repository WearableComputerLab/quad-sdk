...
// TODO: Create Plan class s.t. planner and plan are different concepts
...
OfflineGlobalBodyPlanner::OfflineGlobalBodyPlanner(ros::NodeHandle nh) {
    ...
    robot_state_sub_ = nh_.subscribe(
        robot_state_topic_, 1, &OfflineGlobalBodyPlanner::robotStateCallback, this);
    
    */
    body_plan_pub_ = nh_.advertise<quad_msgs::RobotPlan>("/robot_1/global_plan", 1);
    discrete_body_plan_pub_ = nh_.advertise<quad_msgs::RobotPlan>("/robot_1/global_plan_discrete", 1);

    // Load Planner Configuration
    PlannerConfig planner_config_;
    // Test code (loading and set Misc plan and timestamp should be part of plan class)
    loadCSVData("/home/andrewzheng1011/quad_sdk_ws/src/quad-sdk/offline_global_body_planner/data/states_traj.csv",
                                            "/home/andrewzheng1011/quad_sdk_ws/src/quad-sdk/offline_global_body_planner/data/ctrl_traj.csv",
                                            4, 2);
    setMiscPlans(0, 0.01, state_sequence_, grf_sequence_, t_plan_, primitive_id_plan_, length_plan_, planner_config_);
...
void OfflineGlobalBodyPlanner::setMiscPlans(double t0, double dt, std::vector<State> &state_sequence, std::vector<GRF> &grf_sequence_,
                std::vector<double> &t_plan, std::vector<int> &primitive_id_plan, std::vector<double> &length_plan,
                const PlannerConfig &planner_config) {
    // TODO: Integrate w/ planner_config


    // Check validity of dt
    if (dt <= 0) {ROS_ERROR("Invalid dt = %0.2f. dt must be greater than 0.", dt); }
    ...
    for (int i = 0; i < num_discrete_pts; i++) {
        // Add 3rd dim to state & vel (set pos to nominal height at pos of terrain & vel to 0)
        ...
        if (i == 0) {
            ...
            plan_length = plan_length + poseDistance(state_sequence.at(i-1), state_sequence.at(i));
            length_plan.push_back(plan_length);
        }
    }
...
void OfflineGlobalBodyPlanner::printStateSequence(const std::vector<State> &state_sequence) {
...
void OfflineGlobalBodyPlanner::printGRFSequence(const std::vector<GRF> &grf_sequence) {
...
void OfflineGlobalBodyPlanner::printFullStatePlan(const std::vector<FullState> &full_state_plan) {
...
void OfflineGlobalBodyPlanner::getNominalHeight(Eigen::Vector3d &pos) { // May want to make these functions inline
    pos(2) = 0.25;
}

void OfflineGlobalBodyPlanner::getTerrainZFilteredFromState(const Eigen::Vector3d &pos, const PlannerConfig &planner_config) {
    // TODO: Get pos from fastTerrainMap which is w/in planner_config
    // rn setting terrain height to be hardcoded value (scaling up get from rosparam)
    std::cout << "Hello getTerrainZFilteredFromState" << std::endl;
}

void OfflineGlobalBodyPlanner::addMG(GRF &grf) {
    grf(2) = 12.5*9.81; // TODO: Get from param server **
}
...
void OfflineGlobalBodyPlanner::addStateAndGRFToMsg(double t, int plan_index, const FullState &body_state,
    ...
    bool contact_state = (primitive_id != 2); // TODO: Hardcoded for now... not sure what to do about this
...
void OfflineGlobalBodyPlanner::convertPlanToMsg(quad_msgs::RobotPlan &robot_plan_msg, quad_msgs::RobotPlan &discrete_robot_plan_msg) {
    ...
    for (std::size_t i = 0; i != state_sequence_.size(); i++) {
        ...
        addStateAndGRFToMsg(0.0, 0, full_discrete_state, grf_sequence_[i], primitive_id_plan_[i], discrete_robot_plan_msg);
    }
...
void OfflineGlobalBodyPlanner::loadCSVData(std::string state_file_path, std::string ctrl_file_path, int num_states, int num_ctrl) {

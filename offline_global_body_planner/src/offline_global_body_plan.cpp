...
/**
 * @brief Construct a new Offline Global Body Plan:: Offline Global Body Plan object
 */
...
void OfflineGlobalBodyPlan::addWalkID() {
    // TODO: take in param and add parameter for walk id
}

void OfflineGlobalBodyPlan::getDistToGoal() {
    // TODO: Far future when running online planner
}

void OfflineGlobalBodyPlan::printStateSequence(const std::vector<State> &state_sequence) {
...
void OfflineGlobalBodyPlan::printGRFSequence(const std::vector<GRF> &grf_sequence) {
...
void OfflineGlobalBodyPlan::printFullStatePlan(const std::vector<FullState> &full_state_plan) {
...
void OfflineGlobalBodyPlan::addStateAndGRFToMsg(double t, int plan_index, const FullState &body_state,
...
void OfflineGlobalBodyPlan::convertPlanToMsg(quad_msgs::RobotPlan &robot_plan_msg, quad_msgs::RobotPlan &discrete_robot_plan_msg) {
    // Get body plan size
...
void OfflineGlobalBodyPlan::loadPlanData(double t0, double dt, const FullState &start_state, 
                std::vector<State> &state_sequence, std::vector<GRF> &grf_sequence,
    ...
    for (int i = 0; i < num_discrete_pts; i++) {
        ...
        getNominalHeight(state_sequence.at(i), planner_config);
        state_sequence.at(i).vel[2] = 0; // TODO: Figure out what to do with v_z
    ...
    addFullStates(start_state, state_sequence, 0.01, body_plan_, planner_config); // TODO: add dt

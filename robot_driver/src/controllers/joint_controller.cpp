...
bool JointController::computeLegCommandArray(
  ...
  for (int i = 0; i < num_feet_; ++i) {
    ...
    for (int j = 0; j < 3; ++j) {
      ...
      // ROS_INFO_THROTTLE(0.2, "joint_idx_: %0.2f, joint_idx: %0.2f", joint_idx_, joint_idx);

      if ((i == leg_idx_) && (joint_idx == joint_idx_)) { // Does this condition work? It is hip joint and upper leg joints of FL and RL (joint_idx == j)
        joint_torque_val = std::max(std::min(joint_torque_, 30.0), -30.0); // joint torque val must be between 5 and -5
        ROS_INFO_THROTTLE(0.2, "Leg %d, joint %d, cmd = %5.3f", i, joint_idx_,

...
void GBPL::extractPath(PlannerClass &Ta, PlannerClass &Tb,
  ...
  for (int i = 0; i < action_sequence_b.size(); i++) {
    // std::cout << "i: " << i  << std::endl; // i: [0, 1] mostly for flat
    // terrain... so flipping might make action sequence 2-4
...
int GBPL::findPlan(const PlannerConfig &planner_config, State s_start,
    ...
    if (isValidState(s_rand, planner_config, LEAP_STANCE)) {
      if (extend(Ta, s_rand, planner_config, FORWARD, tree_pub) != TRAPPED) {
        ...
        if (connect(Tb, s_new, planner_config, FORWARD, tree_pub) == REACHED) {
          goal_found = true;
          // std::cout << "Goal Found" << std::endl; // Repeatedly saying goal
          // found... is this not the final goal?? or it only needs to find w/in
          // specific time
  ...
  num_vertices_ = (Ta.getNumVertices() + Tb.getNumVertices());
  // std::cout << "Num Vertices total: " << num_vertices_ << std::endl; // 4 for
  // flat plane... Ta start end, Tb start end, connect
  ...
  // std::cout << "state_sequence size: " << state_sequence.size() << std::endl;
  // // Around 2 for flat terrain std::cout << "action_sequence size: " <<
  // action_sequence.size() << std::endl; // Around 1 for flat terrain
  ...
  for (Action a : action_sequence) {
    // counter = counter + 1;
    // std::cout << "For action sequence..." << counter << std::endl; // 1 loop
    // for flat terrain

...
GlobalBodyPlannerPublisher::GlobalBodyPlannerPublisher(ros::NodeHandle nh) {
  ...
  quad_utils::loadROSParam(
      nh_, "topics/global_plan",
      body_plan_topic);  // Need to consider way to approach this topic l8r
  ...
  */
  ...
  terrain_map_sub_ =
      nh_.subscribe(terrain_map_topic_, 1,
                    &GlobalBodyPlannerPublisher::terrainMapCallback, this);

  robot_state_sub_ =
      nh_.subscribe(robot_state_topic_, 1,
                    &GlobalBodyPlannerPublisher::robotStateCallback, this);
  // goal_state_sub_ = nh_.subscribe(goal_state_topic, 1,
  //                                &GlobalBodyPlannerPublisher::goalStateCallback,
  //                                this);
...
void GlobalBodyPlannerPublisher::robotStateCallback(
...
bool GlobalBodyPlannerPublisher::callPlanner() {
  ...
  start_pos << 0.0, 0.0,
      0.270;  // Make this robust to actual robot starting state
  ...
  hold_grf_0 << 0.0237608, 0.00597425,
      0.271616;  // z component will turn to m*g in convertToMsg
  ...
  replan_start_time_ = 0;   // Start time may need to be gotten from
                            // setComputedTimestamp... do I need this..? maybe

  ros::Time plan_start_timestamp_ = ros::Time::now();

  FullState start_state_ =
      stateToFullState(start_state, 0, 0, 0, 0, 0,
                       0);  // Convert start state to FullState datatype

  // Set up Global Body Plan
  // std::cout << "Computed timestamp: " << plan_start_timestamp_.toSec() <<
  // std::endl; Don't think computedTimestamp is useful... only used as a way to
  // compare if planner is equal to another?
  plan_.setComputedTimestamp(
      plan_start_timestamp_);  // eraseAfterIndex(int n) useful if need to
                               // replan
  plan_.loadPlanData(plan_status, start_state_, dist_to_goal_, state_sequence,
                     action_sequence, dt_, replan_start_time_, planner_config_);
}
...
void GlobalBodyPlannerPublisher::publishPlan() {
  ...
  if (!published_plan) {
    ...
    plan_.setPublishedTimestamp(
        ros::Time::now());  // RN FIGURE OUT WHY TIMESTAMP DON'T MATCH UP
  }

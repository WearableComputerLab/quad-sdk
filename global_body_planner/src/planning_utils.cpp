...
double getDzFromState(const State &s, const PlannerConfig &planner_config) {
  ...
  std::cout << "dot product: " << -s.vel.head<2>().dot(surf_norm.head<2>()) <<
  std::endl;
  */
...
State interpStateActionPair(const State &s_in, const Action &a, double t0,
  ...
  // std::cout << "t0: " << t0 << std::endl; // Output is all 0
...
bool isValidAction(const Action &a, const PlannerConfig &planner_config) {
  ...
  // std::cout << "grf_0: " << grf_0 << std::endl;
  // std::cout << "grf_f: " << grf_f << std::endl; // grf_f prob doesn't get
  // used..

  // std::cout << "a.t_f: " << a.t_f << std::endl; // This is always 0 for even
  // terrain with flight phase???
  if (a.t_f == 0) {
